...
- **Real eval baseline.** `eval_baseline.json` currently reflects a synthetic run. Capture numbers from `make eval-full` against the curated fixtures on real Ollama + Chromium, commit them, and let `--accept-zero-baseline` retire. The regression gate isn't useful until the baseline reflects a real run.
...
- **Test the `Auditor` class directly.** Today `axe_runner`, `llm_client` shape, and the regression gate all have unit coverage. `auditor.audit` is exercised only end-to-end. Add a unit test that injects `MockClient` and a fake violation list, so an `auditor.py` refactor shows up in tests before it shows up in prod.
...
- **Multiple LLM providers.** The `LLMClientProtocol` seam exists and is already used by `MockClient`. Adding `LMStudioClient` or `LocalAIClient` is implementing a second class, not redesigning anything. No cloud APIs, see Won't-do.
...
- **Caching of LLM fixes.** Same violation, same HTML context, same fix. A content-addressed cache keyed by `(rule_id, html_hash)` saves Ollama calls on re-audits of files that haven't changed much. Probably a sqlite table next to `audits.db`.
