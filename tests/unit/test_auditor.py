...
class _StubFixEngine:
    """Protocol-compatible stub used for pure auditor.audit unit tests."""

    def __init__(self) -> None:
        self.calls: list[tuple[str, str, str]] = []

    def generate_fix(self, violation: ViolationInput, html_context: str, file_path: str) -> AuditResult:
        self.calls.append((violation.id, html_context, file_path))
        return AuditResult(
            file_path=file_path,
...
class TestAuditOrchestrationNoE2E:
    def test_audit_uses_stub_engine_with_fake_violations(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Unit-test audit orchestration without Playwright or sidecars."""
        fake_violations = [
            _make_violation("image-alt", ImpactLevel.CRITICAL),
            _make_violation("label", ImpactLevel.SERIOUS),
        ]
        stub_engine = _StubFixEngine()

        monkeypatch.setattr(Auditor, "_get_violations_real", lambda self, _path: fake_violations)
        monkeypatch.setattr("wcag_auditor.auditor.save_report", lambda report: 123)

        auditor = Auditor(fix_engine=stub_engine)
        report = auditor.audit("https://example.com", save=True)

        assert report.total_violations == 2
        assert report.critical_count == 1
        assert report.serious_count == 1
        assert [result.rule_id for result in report.results] == ["image-alt", "label"]
